...
from fsdviz.common.models import (Agency, Lake, Grid10, Species, Strain,
                                  StrainRaw, StateProvince, Mark, LatLonFlag)

from fsdviz.stocking.models import (LifeStage, Condition, StockingEvent,
                                    StockingMethod)

from utils.lwdb_utils import (int_or_None, float_or_None, get_stocking_method,
                              get_condition, pprint_dict, clean_title,
                              get_mark_codes, associate_cwt, get_latlon,
...
lake_centroids = {
    x.abbrev: Centroid(ddlat=x.shoreline.centroid.y,
                       ddlon=x.shoreline.centroid.x)
    for x in Lake.objects.exclude(shoreline__isnull=True)
}

mu_centroids = {x.label:Centroid(ddlat=x.geom.centroid.y,
                                 ddlon=x.geom.centroid.x) for x in mus}
...
for lake in lakes:
    ...
    tmp = {x.grid:Centroid(ddlon = x.centroid.x,
                       ddlat = x.centroid.y) for x in grids}
...
##lifestage lookup:
tmp = LifeStage.objects.all()
lifestage_lookup = {x.abbrev:x for x in tmp}
...
tmp = Agency.objects.all()
agency_lookup = {x.abbrev:x for x in tmp}

tmp = StateProvince.objects.all()
stateprov_lookup = {x.abbrev:x for x in tmp}

tmp = Species.objects.all()
species_lookup = {x.abbrev:x for x in tmp}
...
mdbcur.execute("SELECT DISTINCT [year] FROM [GLFSD] where year <= '1999' order by [year] desc")
rs = mdbcur.fetchall()
...
for yr in years:
    ...
    print("Getting records for {:d}: {:4d} records found".format(
        int(yr), len(rs)))
    ...
    for row in rs:
        ...
        strain_raw = get_or_create_rawStrain(
            species, raw_strain=record['strain'])
        ...
        pt = get_latlon(record, grid_centroids, mu_centroids,
                        lake_centroids)
        dd_lat = pt.get('ddlat')
        dd_lon = pt.get('ddlon')
        ...
        my_event = StockingEvent(
            ...
            condition=get_condition(
                int_or_None(record['condition']), CONDITION),
        ...
        if mark_codes:
            my_marks = Mark.objects.filter(
                mark_code__in=mark_codes.get('codes'))
