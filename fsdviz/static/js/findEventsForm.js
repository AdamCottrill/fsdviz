(function () {
	...
	const get_options = grp => grp.top(Infinity).filter(d => d.value > 0).map(d => d.key); // update the options given a selector, the options, and a lookup table


	const update_options = (selector$$1, option_data, lookup, sortKey = "label") => {
	  let options = select(selector$$1).selectAll("option").data(option_data, function (d) {
	    return d ? d : this.value;
	  });
	  options.exit().remove();
	  let optionsEnter = options.enter().append("option").text(d => lookup[d]).attr("value", d => d);
	...
	let stageDim = ndx.dimension(d => d.stage); // set up our groups
	...
	let stockingMethod_listener = select("#id_stocking_method").on("change", function () {
	  let selected = get_selections(this);
