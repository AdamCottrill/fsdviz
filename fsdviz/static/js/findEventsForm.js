(function () {
  ...
  const h$1 = heapselect_by(identity);
  ...
  var update_options = function update_options(selector$$1, option_data, lookup) {
    var sortKey = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "label";
    var options = select(selector$$1).selectAll("option").data(option_data, function (d) {
  ...
  var roiGroup = roiDim.group().reduceSum(function (d) {
    return d.events;
  });
  ...
  var markGroup = markDim.group().reduceSum(function (d) {
    return d.events;
  });
