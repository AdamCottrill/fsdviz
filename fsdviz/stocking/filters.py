...
class StockingEventFilter(django_filters.FilterSet):
    ...
    agency = django_filters.CharFilter(
        field_name='agency__abbrev', lookup_expr='iexact')
    ...
    strain = django_filters.CharFilter(
        field_name='strain_raw__strain__strain_code', lookup_expr='iexact')

    lifestage = django_filters.CharFilter(
        field_name='lifestage__abbrev', lookup_expr='iexact')

    stocking_method = django_filters.CharFilter(
        field_name='stocking_method__stk_meth', lookup_expr='iexact')

    jurisdiction = django_filters.CharFilter(
        field_name='jurisdiction__slug', lookup_expr='exact')

    # contains??
    mark = django_filters.CharFilter(field_name='mark', lookup_expr='iexact')

    class Meta:
        model = StockingEvent
        fields = [
            'agency__abbrev', 'year', 'species__abbrev',
            'strain_raw__strain__strain_label', 'lifestage__abbrev',
            'jurisdiction__slug', 'stocking_method__stk_meth', 'mark'
        ]
